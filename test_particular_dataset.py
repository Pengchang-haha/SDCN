...
from mainSDSA import RandomCropPatches, NonOverlappingCropPatches
import numpy as np
...
if __name__ == "__main__":
    ...
    elif args.modelname == 'SDSA':
        model = SDSA().to(device)
    ...
    #K = 10
    #k = 10
    #testindex = index[int((k-1)/K * len(index)):int((k)/K * len(index))]#所有的index
    testindex=index[:int(1*len(index))]
    test_index = []
    ...
    krocc = stats.stats.kendalltau(mos, scores)[0]
    print("Test Results - SROCC: {:.4f} PLCC: {:.4f} KROCC: {:.4f}".format(srocc, plcc, krocc))
    np.save(args.save_path, (srocc, plcc,krocc))