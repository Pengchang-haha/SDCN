...
class _NonLocalBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True):
        ...
        if bn_layer:
            ...
            nn.init.constant_(self.W[1].weight, 0)#用0填充向量
    ...
    def forward(self, x):
        ...
        y = torch.matmul(f_div_C, g_x)#对两个矩阵相乘
        y = y.permute(0, 2, 1).contiguous()#y.permute(0, 2, 1)将y的维度索引1和维度索引2交换位置：contiguous方法就类似深拷贝，使得上面这些操作不会改变元数据
...
if __name__ == '__main__':
    ...
    for (sub_sample, bn_layer) in [(True, True), (False, False), (True, False), (False, True)]:
        ...
        img = torch.zeros(2, 3, 20, 20)#返回值都为0的
